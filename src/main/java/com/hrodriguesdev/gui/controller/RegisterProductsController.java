...
//	Menu itens 1
	...
	private void saveAction(ActionEvent e) {		
		...
		try {
			...
			amount =  Double.valueOf( amountField.getText().replaceAll("[^0-9-.]+" ,"") );
		...
		try {
			if(!unitMeasurementBox.getValue().isEmpty())
				idUnit = obsUnidade.stream().filter(a -> a.getUnidade().equals(unitMeasurementBox.getValue())).findFirst().orElseThrow(() -> new ExceptionAlfa("Unidade nao encontrada")).getId();
		...
		if(product != null) {
			product = new Product(product.getId(), productFieldTxt, descriptionFieldText, valuePaid, valueSale, amount, idUnit);
			...
			Product product = new Product(productFieldTxt, descriptionFieldText, valuePaid, valueSale, amount, idUnit);
			created = controllerProd.createdNew(product);
			if(created) {
				Alerts.showAlert("Criado Produto", "Produto "+ product.getName() +", criado com sucesso!" , "", AlertType.INFORMATION);
			}
		}
		cancelarAction(new ActionEvent());
	}
	...
	private void editAction(ActionEvent e) {
		if(!productTable.getSelectionModel().isEmpty()) {
			...
			amountField.setText(product.getQtde().toString());	
...
//					delete = controller.delete(product.getId());					
				...
				if(delete)
					Alerts.showAlert("Produto", "Produto - " + product.getName() + " Deletada com sucesso", "", AlertType.INFORMATION);
			}
	...
	private void filtredDouble(KeyEvent eventKey) {
		if(eventKey.getTarget().equals(valuePaidField)) {
			...
			valueSaleField.end();
		}else if(eventKey.getTarget().equals(amountField)) {
